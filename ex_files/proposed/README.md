# Proposed Schemas — Sales ROI Helper (v2)

ไฟล์ในโฟลเดอร์นี้คือ **schema ที่เสนอ** (ยังไม่ถูก migrate เข้า Google Sheet จริง)
อ่านคู่กับ `../docs/sales_roi_helper_design.md` (v2)
...
| `rate_benchmark.csv` | MRR/MLR/MOR/EIR (จะ sync จาก BOT API) | (ไม่มีของเดิม) |
...
> **v2 changelog**: ตัด `recommend_rule.csv` ออกแล้ว — เปลี่ยนเป็น System Comparison (คำนวณทุกระบบ) + Financing Rank (algorithm ในโค้ด) ตาม feedback รอบที่ 1
> **`solar_products`** — ใช้ schema เดิมจาก `package_ex.xlsx` ตามที่ตกลง ไม่มี seed ใหม่ในโฟลเดอร์นี้
...
- ตัวเลข `MRR=0.066`, `MLR=0.064` ใน `rate_benchmark.csv` เป็นค่าตัวอย่าง — ระบบจริงจะ sync จาก BOT Open API รายเดือน (ดู design doc §7)
