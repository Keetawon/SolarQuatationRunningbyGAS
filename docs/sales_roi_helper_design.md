...
## Changelog v2 → v2.1 (feedback รอบ 2)

- ➕ **เพิ่ม Step 0: Start** — ก่อนเข้า wizard ให้ค้นหา session ที่เคยทำหรือสร้างใหม่ (ค้นจาก `customer_label`)
- 📌 ยืนยัน default `authority = MEA` — เซลล์เปลี่ยนเป็น PEA ได้ในขั้น 1 (อธิบายในเอกสารว่า MEA = กทม/นนท์/สป. เท่านั้น; ปริมณฑลอื่น เช่น ปทุม/นครปฐม/สป.สาคร = PEA)
- 📌 PSH/PR ใช้ **DEFAULT เดียว (4.5 / 0.85)** เพราะลูกค้า ~100% อยู่ กทม-ปริมณฑล; เก็บ `roi_assumption.region` ไว้แต่ไม่ทำ dropdown ในขั้น 1 (อยู่ใน "ค่าตั้งต้นขั้นสูง" collapsible เท่านั้น)
- 📌 ยืนยัน Custom kWp รองรับขนาดใดก็ได้ (ไม่จำกัดเฉพาะ 3/5/7/10)
- 📌 ยืนยัน Autosave debounce 2-3 วินาที
- 📌 ยืนยัน FiT default OFF
...
│                                              │
│  0. หน้าเข้า (Start)                          │
│     ┌─ ค้นหาลูกค้าที่เคยทำไว้ ──────────┐    │
│     │ 🔎 [พิมพ์ "87/75" หรือ "ธนวัฒน์"]  │    │
│     │ → list session ที่ match (แสดง     │    │
│     │   วันที่ล่าสุด, สถานะ, ผลิตภัณฑ์)    │    │
│     └─────────────────────────────────────┘   │
│     ┌─ [ + สร้าง session ใหม่ ] ───────┐     │
│                                              │
...
│     • เขตการไฟฟ้า:                            │
│         ⦿ MEA (กทม/นนท์/สป.) — default       │
│         ○ PEA (ที่เหลือ)                      │
│       ⓘ "กรุงเทพ นนทบุรี สมุทรปราการ ใช้ MEA  │
│          ปริมณฑลอื่น/ต่างจังหวัด ใช้ PEA"      │
│                                              │
│  2. ค่าไฟ Anchor                             │
...
| `searchRoiSessions(query, limit=20)` | partial `customer_label` (เช่น "87/75" หรือ "ธนวัฒน์") | array `{session_id, customer_label, status, last_updated, selected_kw, monthly_net}` — เรียง last_updated desc |
| `loadRoiSession(session_id)` | — | full session + appliance lines |
| `inferKwhFromBill({bill, authority})` | — | `{kWh, avgRate, tariff_id}` |
| `syncBotRates()` | — | refresh `rate_benchmark` จาก BOT API (ดูข้อ 7) |

> **Search กลยุทธ์**: ใช้ case-insensitive `substring match` บน `customer_label` ของ `roi_session` ใน Sheet
> ขนาดข้อมูล: ถ้า session ≤ 5,000 records → อ่านทั้งหมดใน 1 call (`getValues()`) แล้ว filter ใน JS — เร็วพอ ไม่ต้อง index แยก
> ถ้าโตเกิน 10k → เพิ่ม sheet `roi_session_index` (customer_label_lc, session_id) เป็น lookup โดย onEdit trigger
...
## 10. Open Questions — ตอบครบแล้ว ✓

> รอบ 1 ✓ / รอบ 2 ✓ — พร้อม implement Phase 1

| # | คำถาม | คำตอบ | ผลต่อ design |
|---|---|---|---|
| 1 | session ใหม่ทุกครั้ง หรือ "ค้นลูกค้าที่เคยทำไว้"? | **ค้นจาก `customer_label`** เช่น "87/75 - ว่าที่ร้อยตรี ธนวัฒน์ ยุบลเขตร์" | เพิ่ม Step 0 (หน้า Start) + endpoint `searchRoiSessions` + `loadRoiSession` |
| 2 | default MEA/PEA? | **MEA** (เพราะลูกค้าส่วนใหญ่อยู่ กทม/นนท์/สป.); ปริมณฑลอื่น/ต่างจังหวัด toggle เป็น PEA | step 1 มี radio พร้อม tooltip อธิบาย |
| 3 | PSH/PR ผูกกับภาคหรือไม่? | **ใช้ DEFAULT เดียว (4.5/0.85)** เพราะลูกค้า ~100% อยู่ กทม-ปริมณฑล | ไม่มี dropdown ภาคในขั้น 1; เก็บไว้ใน "ค่าตั้งต้นขั้นสูง" ถ้าเซลล์อยากปรับ |
| 4 | custom kWp ที่ไม่อยู่ในชั้น? | **รองรับขนาดใดก็ได้** | step 5 ปุ่ม "เพิ่มระบบ custom" รับ kWp ทศนิยม |
| 5 | Autosave 2-3 วินาที vs ตอน next step? | **Autosave** | debounced sync ทุก 2 วินาที |
| 6 | FiT default OFF? | **OFF** | checkbox unticked เป็นค่าเริ่มต้น |
